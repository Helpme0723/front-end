...
export const deletePost = async postId => {
  try {
    ...
    const err = new Error(
      error.response?.data?.message || '포스트 삭제 중 오류가 발생했습니다.',
    );
...
export const getPostLikeCheck = async postId => {
  try {
    const response = await axiosInstance.get(`api/posts/${postId}/like/check`, {
      params: { postId },
    });
...
export const getCommentLikeCheck = async postId => {
  try {
